...
import { store, delet, update, findByName, findByI} from './controller/userController';
import { db, UserSequelize } from './database';
...
route.post('/login', async (request: Request, response: Response) => {
    ...
    if(email != undefined && password != undefined){    
        const user = await UserSequelize.findOne({where: {email: email},
                ...
                jwt.sign({
                 ...
                 (err, token) => {
                    if(err){
                        ...
                        return response.json({token: token, email: user2.email, message: 'Login realizado com sucesso'})
                    }
                 }
                )
        ...
        return response.status(400).json({message: 'Email e/ou Senha não podem ser nulo'})          
    }
})
